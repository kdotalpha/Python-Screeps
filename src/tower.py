...
def run_tower(tower):
    ...
    if not target and tower.store.getUsedCapacity(RESOURCE_ENERGY) > globals.TOWER_ENERGY_RESERVE_PERCENTAGE[tower.pos.roomName] * tower.store.getCapacity(RESOURCE_ENERGY):
        ...
        if not target:
            ...
            target = globals.getBrokenStructure(tower, True, 0.0001, True, None, STRUCTURE_RAMPART)
        if not target:
            #heal walls to bare minimum
            target = globals.getBrokenStructure(tower, True, 0.0001, False, None, STRUCTURE_WALL)
        if not target:
